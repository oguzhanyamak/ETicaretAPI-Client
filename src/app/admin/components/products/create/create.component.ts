...
export class CreateComponent {
  ...
  create(data: { name: string, stock: number, price: number }) {
    ...
    if (this.frm.valid) {
      this.productService.create(createProduct, () => { this.toasterService.message("Ekleme İşlemi Başarıyla Yapılmıştır", "Başarılı", MessageType.Success, ToastrPosition.TopRight)},(errorMessage:string)=>{this.toasterService.message(errorMessage,"Hata",MessageType.Error,ToastrPosition.BottomFullWidth)});
    }
    else {
      this.toasterService.message("Lütfen Formu Uygun Doldurunuz", "Başarisiz", MessageType.Error, ToastrPosition.TopRight)
    }
